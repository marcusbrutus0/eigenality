...
async fn test_post_method_body_interpolation_via_resolve_item_data() {
    ...
    let mut fetcher = eigen::data::DataFetcher::new(&sources, root, None, pool, None, false);
...
<img src='{{ source_asset("test_cms", "/uploads/hero.png") }}'>
    ...
    // Build.
    eigen::build::build(root, false, true, false).await.unwrap();
...
// Utility
// ============================================================================
// Redirect rules (_redirects file generation)
// ============================================================================

/// Config [[redirects]] rules are written to dist/_redirects.
#[tokio::test]
async fn test_redirects_generated_from_config() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
...
"#,
    ...
    let gone_pos = content.find("/gone").unwrap();
    assert!(old_pos < gone_pos, "Rules must appear in config order");
}

/// A static/_redirects file is copied verbatim when no config rules are present.
#[tokio::test]
async fn test_redirects_static_file_copied() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
...
#[tokio::test]
async fn test_redirects_conflict_aborts_build() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path();
